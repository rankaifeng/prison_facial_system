...
class RecordService(BaseService):
    ...
    def get_prison_messages(page=1, page_size=20, prison_area=None):
        """获取今日释放人员列表 - 从档案库查询今天刑满释放的人，用is_released判断状态"""
        from apps.users.models import PrisonerArchive
        ...
        logger.info(f'[今日释放人员] 查询日期: {today_str}, prison_area: {prison_area}')
        ...
        archive_records = []
        for r in qs:
            sentence_end = r.sentence_end or ''
            # 处理不同日期格式：2026-08-05, 2026.08.05, 2026/08/05
            match = re.match(r'(\d{4})[-./年](\d{1,2})[-./月](\d{1,2})', sentence_end)
            if match:
                normalized = f"{match.group(1)}-{int(match.group(2)):02d}-{int(match.group(3)):02d}"
                if normalized == today_str:
                    archive_records.append(r)

        logger.info(f'[今日释放人员] 找到 {len(archive_records)} 条记录')

        total = len(archive_records)
        start = (page - 1) * page_size
        end = start + page_size
        page_records = archive_records[start:end]

        data = [{
            'id': r.id,
            'prisoner_no': r.prisoner_no,
            'prisoner_name': r.prisoner_name,
            'prison_area_name': get_prison_area_name(r.prison_area) or r.prison_area or '',
            'reason': '刑满释放',
            'status': '已释放' if r.is_released else '未释放',
