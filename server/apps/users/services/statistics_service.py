...
class StatisticsService(BaseService):
    ...
    def check_and_reset_daily_stats():
        ...
        """
        from apps.users.models import DailyStatistics, HistoryStatistics, TodayExitRecord, PrisonerArchive
        ...
        # 如果今天仍然没有 DailyStatistics（昨天也没有数据），从档案库创建默认记录
        if not DailyStatistics.objects.filter(date=today).exists():
            # 从档案库获取各监区在押人数
            archive_by_area = PrisonerArchive.objects.filter(is_released=False).values('prison_area').annotate(count=Count('id'))
            from apps.users.dict import get_prison_area_name
            for item in archive_by_area:
                area_id = item['prison_area']
                if not area_id:
                    continue
                area_name = get_prison_area_name(area_id) or area_id
                DailyStatistics.objects.get_or_create(
                    prison_area=area_id,
                    date=today,
                    defaults={
                        ...
                        'exit_count': 0,
                        'entry_count': 0,
                        'in_prison_count': item['count'],
                        'work_count': 0,
                        'reason_stats': {},
                    }
                )
            logger.info(f'[auto-reset] 从档案库创建今日统计 ({len(archive_by_area)} 个监区)')
