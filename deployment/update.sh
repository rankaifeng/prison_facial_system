...
if [ -n "$SERVER_IP" ]; then
    # 确保 ALLOWED_HOSTS 为 *
    if ! grep -q "ALLOWED_HOSTS=\*" "$INSTALL_DIR/docker-compose.yml"; then
        sed -i "s/ALLOWED_HOSTS=.*/ALLOWED_HOSTS=*/" "$INSTALL_DIR/docker-compose.yml"
        echo "  已设置 ALLOWED_HOSTS=*"
    fi
...
# ── 手持终端 WebSocket 转发（8081 → 8000）──
echo ""
echo "[3.5/6] 配置手持终端端口转发 (8081 → 8000)..."
apt-get install -y socat >/dev/null 2>&1 || yum install -y socat >/dev/null 2>&1 || true

cat > /etc/systemd/system/prison-ws-forward.service << 'SOCEOF'
[Unit]
Description=Prison WebSocket Port Forward 8081->8000
After=network.target

[Service]
Type=simple
ExecStart=/usr/bin/socat TCP-LISTEN:8081,fork,reuseaddr TCP:127.0.0.1:8000
Restart=always
RestartSec=3
...
systemctl daemon-reload
systemctl enable prison-ws-forward
systemctl restart prison-ws-forward
echo "  端口转发已启动: 8081 → 8000"
